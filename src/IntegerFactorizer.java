...
public class IntegerFactorizer {
	...
	public static BigInteger pollardPMinusOneStrongN(int size, int primalityCertainty) {
		...
		do {
			...
			p = p1.multiply(BigInteger.TWO).add(BigInteger.ONE);
				...
				|| !(p1.isProbablePrime(primalityCertainty) && p.isProbablePrime(primalityCertainty)));
		...
		do {
			...
			q = q1.multiply(BigInteger.TWO).add(BigInteger.ONE);
				...
				|| !(q1.isProbablePrime(primalityCertainty) && q.isProbablePrime(primalityCertainty)));
	 ...
	 * @return
	 */
	public static BigInteger dixon(BigInteger n, int primalityCertainty) {
		boolean b1, d1;
		BigInteger c, p, q;
		c = n.sqrt();
		int f1 = 0;

		for (BigInteger f = c.add(BigInteger.ONE); f.compareTo(n) <= 0; f = f.add(BigInteger.ONE)) {
			BigInteger s;
			BigInteger res[] = f.modPow(BigInteger.TWO, n).sqrtAndRemainder();
			s = res[0];
			...
			if (remainder.equals(BigInteger.ZERO)) {
				...
				if ((remainder.equals(BigInteger.ZERO))) {
					...
					if (s.compareTo(f.mod(n)) != 0) {
						p = f.add(s);
						q = f.subtract(s);
						b1 = p.isProbablePrime(primalityCertainty);
						d1 = q.isProbablePrime(primalityCertainty);
						if (b1) {
							BigInteger z = q.gcd(n);
							boolean b = z.isProbablePrime(primalityCertainty);
							if (b) {
								return p;
							}
						} else if (d1) {
							BigInteger z = p.gcd(n);
							if (z.isProbablePrime(primalityCertainty)) {
								return p;
							}
						}
						return null;
